...
| `qloopsim.py` | a session shaped like qloop actually runs | sizing `parallel` for the real loop |
...
python3 qloopsim.py -n 3 --rounds 2 --max-tokens 1024
```
...
**Size `parallel` to the real fan-out, which is small.** qloop's own defaults
(`quality-loop/src/quality_loop/cli.py`) are `-n 3` (gate caps at 3) and `-n 4`,
with `--rounds` sequential — so peak concurrency against the qwen instance is 4,
not the 16-32 a synthetic benchmark invites. Matching them measured 18% faster
end-to-end (71.1s -> 58.6s per `best_of_n n=3 rounds=2` session).
